...
add_subtour_cuts(model, sets::Set{Tuple{Si, Int, Vi}}, A::Arcs) = add_cuts(model, [(sum(model[:x][a] for a in δ⁺(A, S)), model[:z][i] + model[:y][block] - 1) for (S, i, block) in sets])
...
function lazy_separation(data::SBRPData, Vb::Si, info, model, cb_data::CPLEX.CallbackContext, context_id::Clong)
  ...
  x_val, y_val, z_val = callback_value.(Ref(cb_data), x), callback_value.(Ref(cb_data), y), callback_value.(Ref(cb_data), z)
...
#    info["iteration_" * string(iteration) * "_time"], info["iteration_" * string(iteration) * "_cuts"], iteration = time, length(sets′), iteration + 1
    ...
    # add ineqs
    add_subtour_cuts(model, components, A)
  end
  return components
end
...
function build_model_sbrp_max(data::SBRPData, app::Dict{String,Any})
  ...
  function create_model(relax_x::Bool = false, relax_y::Bool = false, relax_z::Bool = false)
    ...
    # subtour cuts
    add_subtour_cuts(model, subtour_cuts, A) 
    # registries
